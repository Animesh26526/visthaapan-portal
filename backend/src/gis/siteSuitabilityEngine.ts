  ...
  try {
    ...
    for (const site of sites) {
      ...
      const summaryText = isInsideRedZone
        ? `RESTRICTED: Site intersects active statutory Red Zone (${intersectingRedZones.map(z => z.name).join(', ')}). Permanent relocation prohibited.`
        : `CONDITIONALLY SUITABLE: Site is verified outside all mapped Red Zones. Road corridor is ${isRouteFeasible ? 'open' : 'compromised'}. Nearest hospital is ${hospitalDistKm} km away (${nearestHospital?.hospital_name || 'unassigned'}). Terrain evaluation is UNAVAILABLE pending regional DEM tiles.`;
...
export async function getRelocationSitesGeoJson(): Promise<GeoJsonFeatureCollection> {
  ...
  return {
    ...
    metadata: {
      ...
      source: 'PostGIS relocation_sites + site_suitability_assessments',
    },
  };
}
  ...
  return {
    ...
    explainability: {
      ...
      details: [
        ...
        `Road corridor: ${r.road_accessibility || 'Accessible'}`,
        `Nearest facility: ${r.hospital_name || 'Unmapped'} (${hospitalDistM ? (hospitalDistM / 1000).toFixed(1) + ' km' : 'N/A'})`,
        `DEM status: ${r.terrain_status || 'UNAVAILABLE'}`,
      ],
    },
    dataProvenance: {
      siteSource: 'Chamoli Demonstration Relocation Benchmark (SIMULATED)',
      hospitalSource: 'National Hospital Directory (10,843 Geocoded Facilities)',
      demStatus: 'UNAVAILABLE — Gujarat tiles do not cover Chamoli (Section 8)',
